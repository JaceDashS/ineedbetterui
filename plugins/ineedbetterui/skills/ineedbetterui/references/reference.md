...
1. 프로젝트 폴더를 작업 폴더로 두고 `node <스킬 폴더>/ineedbetterui.mjs`를 백그라운드로 실행하고 출력된 주소를 쓴다. 주소를 모르거나 새 세션이면 같은 명령을 다시 실행한다. 서버가 실행 중이면 주소만 출력하고 끝난다.
...
| 호출 | CLI·IDE 확장: `$ineedbetterui` (목록은 `/skills`). 데스크톱 앱: `@`를 입력하고 스킬 선택 |
