...
스킬을 부르는 것 자체가 기록을 시작하라는 요청이다.

- 다른 요청 없이 스킬만 불렸으면 무엇을 할지 되묻거나 확인을 구하지 않는다. 곧바로 [시작](#시작) 절차대로 서버를 시작하거나 실행 중인 서버를 찾아 주소를 알려 준다. 스킬만 부른 메시지는 질문 entry로 기록하지 않는다.
- 스킬과 함께 다른 요청이 있으면 먼저 서버를 시작한 뒤 그 요청을 처리하고, 그 질문과 응답부터 기록한다.
...
브로드캐스트는 기본으로 꺼져 있고 `127.0.0.1`에만 바인드한다. 사용자가 화면의 설정(사이드바 톱니바퀴)에서 켜면 서버가 프로세스를 다시 시작하지 않고 바인딩만 `0.0.0.0`으로 바꾼다. 포트와 기록은 그대로다.

- 에이전트가 임의로 켜지 않는다. 켜고 끄는 API는 이 PC의 화면에서만 받는다.
- 켜짐·꺼짐은 `broadcast` 이벤트로 남아 `sync.unseen`으로 전달된다. 상태가 바뀐 것을 보면 사용자에게 접속 주소를 알려 준다.
- 켜져 있는 동안에는 인증이 없어 같은 네트워크의 누구나 기록을 보고 쓰기·초기화할 수 있다. 사용자가 켤 때 이 점을 함께 알린다.
...
응답 글자수 한도는 화면 설정 패널(사이드바 톱니바퀴)의 `Max response chars`에서 정한다. 성공·실패 응답 모두에 현재 설정이 담긴 `state`가 들어 있으므로 따로 조회하지 않는다. 기본값은 3000이며 `0`은 무제한이다. 한도를 미리 조회하지 않고 기록한다. 한도 초과로 거부되면 반환된 `maxResponseChars`에 맞춰 임의로 자르지 말고 나누거나 다시 작성해 보낸다. 글자수는 `Array.from` 기준 Unicode code point 수다.
